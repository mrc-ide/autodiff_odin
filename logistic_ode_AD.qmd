...
points(t_obs, d_df$observed[-1], pch=19, col="black") #first element of d_df is t=0 and observed=NA 
for(i in seq_along(t_obs)){
  lines(c(t_obs[i],t_obs[i]), c(N_obs[i],d_df$observed[i+1])) #first element of d_df is t=0 and observed=NA 
}
```
...
```{r}
#| echo: TRUE
gradient_analytical <- likelihood_gradient(K, N0, r, sd_noise, d_df$observed[-1], t_obs) #first element of d_df is t=0 and observed=NA 
gradient_analytical
```
...
- Building the graph of dependencies of the main odin model can be done the same way than for the discrete case
...
\begin{cases}
\begin{align*}
\frac{da_{\theta}(t)}{dt} &= a_{\theta}(t)^{\intercal} \frac{\partial f(z(t),t,\theta)}{\partial \theta}\\
a_{\theta}(0) &= a_{t_{1}}
\end{align*}
\end{cases}
:::
...
- Resulting code (including reversing)
```{r, eval=FALSE}
#| echo: TRUE
