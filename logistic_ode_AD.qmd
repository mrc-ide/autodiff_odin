...
# Principles {background-color="#153E70"}
...
# Worked example: logistic function {background-color="#153E70"}
...
# Analytical derivation {background-color="#153E70"}
...
After a pass forward, we need to through the observation points backward in time going from one state to the other using a system of ODEs containing:

(1) the states;
(2) the ajoints of the states;
(3) the adjoints of parameters (apart from compare or initial conditions ones)

![](images/ode_AD_chen.png){.absolute top=480 left=500 width="489" height="338"}
...
## Backward state equations {.smaller}
...
- At the same time, we run the adjoint state equations
...
D(ff, "N")
```
- Which results in the following adjoint state equation
```{r, eval=FALSE}
#| echo: TRUE
deriv(adj_N) <- adj_N * r * (1- 2*N/K)
initial(adj_N) <- adj_N_end
```
- If we had more states, we would look at which other states the RHS depend on

## Adjoint parameters equations {.smaller}

- Parameter "continuous" accumulation as integration
...
$\frac{da_{\theta}(t)}{dt}= a(t)^{\intercal} \frac{\partial f(z(t),t,\theta)}{\partial \theta}$ 
:::
...
- R code to loop from final observation through observation until initial time
```{r, eval=FALSE}
#| echo: TRUE
for(i in seq_along(t_obs)){
...
## Initialising the states and adjoints
...
## Two functions giving the contributions of the data

- Flowing to state adj_N
```{r}
#| echo: TRUE
...
- Flowing to observation parameter $\sigma$
```{r}
#| echo: TRUE
...
# Wrapping up {background-color="#153E70"}
