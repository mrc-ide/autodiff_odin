...
## Autodiff gradient (3) {.smaller}

- The next step is to derive the equations for the adjoint states $a_{N}(t) = \frac{\partial L}{\partial N(t)}$

- The "continuous" chain rule involves the Jacobian $\frac{\partial f(z(t),t,\theta)}{\partial z}$ with only one state variable here so
...
## Initialising the state and adjoints
```{r}
#| echo: TRUE
