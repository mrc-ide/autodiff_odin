...
D(ff, "N")
```
- Which results in the (simplified) following adjoint state equation
```{r, eval=FALSE}
#| echo: TRUE
deriv(adj_N) <- adj_N * r * (1- 2*N/K)
initial(adj_N) <- adj_N_end
```
- If we had more states, we would look at which other states the RHS depend on using the dependency graph

## Building the dependency graph

Building the graph of dependencies of the main odin model is done the same way than for the discrete case
